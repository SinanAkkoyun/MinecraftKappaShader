...
void main() {
    ...
    if (clamp(screenPosition.xy, pixelSize, 1.0 - pixelSize) == screenPosition.xy && viewDirection.z < 0.0) {
        ...
        if (distanceTraveled < 1e-2) {
            ...
            reflectionCapture   = texture(colortex12, sphereCoord, 0);
            ...
            captureScenePos.xyz = texture(colortex13, sphereCoord, 0).rgb + cameraPosDelta;
            captureScenePos.xyz = reprojected;

            captureScenePos.w   = clamp16F(texelFetch(colortex13, spherePixel, 0).a + distanceTraveled);

            if ((distanceTraveled > 32.0 || distance(sphereCoord, uv) > 0.16) ||
    ...
    reflectionCapture   = clamp16F(vec4(skybox, 1.0));
