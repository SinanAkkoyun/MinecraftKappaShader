...
vec4 atrousSVGF(sampler2D tex, sampler2D gData, vec2 uv, vec3 sceneNormal, float centerDepth) {
    ...
    ivec2 PosNonScaled  = ivec2(uv * viewSize);
    ...
    for (int i = 0; i<25; i++) {
        ...
        ivec2 samplePos     = PosNonScaled + deltaPos;

        bool valid          = all(greaterThanEqual(samplePos, ivec2(0))) && all(lessThan(samplePos, ivec2(viewSize)));

        if (!valid) continue;

        vec4 currentData    = texelFetch(gData, clampTexelPos(ivec2(samplePos * indirectResScale)), 0);
        ...
        vec4 currentColor   = texelFetch(tex, clampTexelPos(ivec2(samplePos * indirectResScale)), 0);
