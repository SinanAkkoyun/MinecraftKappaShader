...
vec2 vertexWindEffect(vec3 pos, const float amp, const float size) {
    ...
    vec2 microWave = vec2(0.0);
        ...
        microWave.x += 0.2 * (1-RWeatherParams.z);
        microWave *= mix(max(windWave(p*0.05, 0.1) + 0.6, 0.0)*0.5+0.2, 1.0, RWeatherParams.z);

    return (macroWave * 0.33 + microWave * (0.6 + RWeatherParams.z * 1.5)) * vec2(-1.0, 1.0) * 0.75 * amp * windIntensity;
}