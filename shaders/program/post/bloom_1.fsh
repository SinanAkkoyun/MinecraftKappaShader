...
vec3 gauss_1d(vec2 uv, vec2 dir, float alpha, int steps) {
    ...
    for (int i = -steps; i<steps+1; i++) {
        ...
        #if pass == 0
            result    += vec4(textureLod(colortex3, spcoord, 0).rgb, 1.0)*weight*float(spcoord.x>mincoord && spcoord.x<maxcoord);
        #elif pass == 1
            result    += vec4(textureLod(colortex3, spcoord, 0).rgb, 1.0)*weight*float(spcoord.y>mincoord && spcoord.y<maxcoord);
        #endif
    }
    ...
    for (uint x = 0; x < downsampleScale.x; ++x) {
        for (uint y = 0; y < downsampleScale.y; ++y) {
            uvec2 pos   = (startPos + ivec2(x, y)) * downsampleScale;
                pos     = clamp(pos, uvec2(0), uvec2(viewSize * sqrt(scaleMult)) - 1);
...
void main() {
    ...
    if (uv.x < scaleMult && uv.y < scaleMult) lumaDownscale = clamp16F(getLuminance4x4(colortex3));
        ...
        if (gl_FragCoord.y*pixelSize.y > 0.22) blur = textureLod(colortex3, gl_FragCoord.xy*pixelSize, 0).rgb; 
